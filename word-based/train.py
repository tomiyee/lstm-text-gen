...
if __name__ == "__main__":
    ...
    try:
        opts, args = getopt.getopt(argv,"hd:c:e:l:n:p:",["dataset=","checkpoints=","epochs=","load_model=","name="])
    ...
    for opt, arg in opts:
        ...
        # Help Command
        if opt == '-h':
            print ('train-model.py -d <dataset> -c <epochs to checkpoint> -e <# epochs> -l <model to load from> -n <name of files> -p <punctuation to allow the model to train on>')
            sys.exit()
        ...
        # white lists the following set of punctuation, ex input "... -p ., ..." would allow "." and ","
        elif opt in ("-p", "--punctuation"):
            whitelisted_punctuation = arg
...
if load_existing_model:
    ...
    def clean_doc(doc):
        ...
        doc = doc.replace('--', ' ')
        doc = doc.replace('-', ' ')
        # adds a space before all whitelisted punctuation marks
